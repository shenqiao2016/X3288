...
static void init_chip(struct i2c_client *client)
{
	...
	msleep(20); 		
	//rc = test_i2c(client);	//hdc 20150424
	//if(rc < 0)
	//{
...
static int gsl_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct gsl_ts *ts = i2c_get_clientdata(client);
  	printk(KERN_ERR "I'am in gsl_ts_suspend() start\n");
...
static void gsl_ts_early_suspend(struct early_suspend *h)
{
	...
	printk("[GSL1680] early Enter %s\n", __func__);
	gsl_ts_suspend(ts->client);
}

static void gsl_ts_late_resume(struct early_suspend *h)
{
	struct gsl_ts *ts = container_of(h, struct gsl_ts, early_suspend);
	printk("[GSL1680] late Enter %s\n", __func__);
...
if (gpio_is_valid(ts->rst_pin)) {
	...
	printk(KERN_ERR "reset pin invalid\n");
}
/*
if (gpio_is_valid(ts->rst_pin)) {
...
{
    int ret=0;
	printk(KERN_ERR "gsl_1680_ts_init...... \n");	
